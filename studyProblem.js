...
易于理解
只测试一个独立单元的工作
在添加新特性后，可以快速测试原有特性是否 由于本次更改产生问题
...
所以一开始先从黑盒用例来写
我们在bugfix(漏洞补缺)或者feature addition的过程中可以逐步增加测试用例
...
fix：目前看来完全没有必要，反正也是通过一种方法来进行解析，需要注意的就是让使用者明白这是个啥，
由于大多数的用户使用习惯，可以添加自定义描述加这个组件的 TYPE,
...
1.在React中 使用JSX 是因为使用了React的语法糖， 所以每页都要引入    17.0以后 和babel合作 减少此操作

2.在vue中 直接使用了 .vue的文件， 按我目前理解，应该是node做了一个转换
...
*
*
* 1.axios 其实你可以直接使用他的实例，
*
* 一、封装axios
* 封装方法  （面向对象注册接口）  函数式变成 注册接口
* 需要request请求配置       //统一的进行 数据格式化
* 相应response 接受数据处理 //统一的进行 数据格式化   这么做的唯一目的无非就是简单，可以添加额外扩展，常用方法，还是一个callback
* 了解http特性(已习得)
* 封装 特殊化code  自定义错误
...
* nginx 线上配置 简单应用
*
* 正常的部署就是  网上出售的域名，就是他服务器的的一个 域名 ip 对应表而已
* 在发现当页面的url后 浏览器进行network请求 根据contType，如果是html的话，那么就开始 dom构建了 进入了 renderer process
*
*
*
...
由于用户访问源站业务有性能瓶颈，通过cdn技术把源站的内容缓存到多个节点。用户
...
一个自定义脚本，可以通过命令来创建文件夹

这个东西的体现就是 在没有可视化界面的时候，人们通过命令 来控制电脑
...
/*需要学习http/https/http2  我在做项目中应该怎么使用   目前看主要是 在分开发环境和生产环境 使用http 和HTTPS 来区分
  ...
  #HTTPS
    ...
    HTTPS 相对于 HTTP  添加了 SSL/TLS加密
     ...
     #HTTPS缺点与解决
       如果每一https的请求都持续进行TLS握手，TLS的握手那么复杂，势必会对通信带来较大的延时，这对于注重用户体验的网站来说，
       是不可接受的。
  ...
  *  浏览器缓存策略
    ...
    @协商缓存
        ...
        缓存是否已过期
        1.如果没有过期那么直接从本地读取，不会产生http请求，此为 强缓存
 ...
 *    非同源请求，均为跨域
 *    #什么是同源
 *      如果两个页面拥有相同的协议（protocol）,）主机(host),端口（port),那么这两个页面就属于同一个源（origin） 同源策略
 *  2.跨域有哪些方法
 *    #1.jsonp
